...
public class GridUtils
{
	...
	{
		if(direction == CellGrid.DIR_HORIZONTAL)
		{
			return new GridCoordinate(gc.getX() + 1, gc.getY());
		}
		else if(direction == CellGrid.DIR_VERTICAL)
		{
			return new GridCoordinate(gc.getX(), gc.getY() + 1);
		}
		else if(direction == CellGrid.DIR_DIAG_UP)
		{
			return new GridCoordinate(gc.getX() + 1, gc.getY() - 1);
		}
		else if(direction == CellGrid.DIR_DIAG_DOWN)
		{
			return new GridCoordinate(gc.getX() + 1, gc.getY() + 1);
		}
		else if(direction == CellGrid.DIR_R_HORIZONTAL)
		{
			return new GridCoordinate(gc.getX() - 1, gc.getY());
		}
		else if(direction == CellGrid.DIR_R_VERTICAL)
		{
			return new GridCoordinate(gc.getX(), gc.getY() - 1);
		}
		else if(direction == CellGrid.DIR_R_DIAG_UP)
		{
			return new GridCoordinate(gc.getX() - 1, gc.getY() - 1);
		}
